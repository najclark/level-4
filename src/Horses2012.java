...
public void testPartA() throws Exception {
		HorseBarn horseBarn = HorseBarn.createTestBarn();
		assertEquals(0, horseBarn.findHorseSpace("Trigger"));// 0 A horse namedTrigger is in space 0.
		assertEquals(2, horseBarn.findHorseSpace("Silver")); // 2 A horse named Silveris in space 2.
		assertEquals(-1, horseBarn.findHorseSpace("Coco"));// ­-1 A horse named Cocois not in the barn
}
